...
- [Architecture](../docs/cosmic-ar-architecture.md) — supervisor + ten
...
[`flows/`](flows/) holds 11 LangFlow export skeletons: the **wired** supervisor
flow, the **wired** File Intake Flow (`ar_file_intake`, the 10th subflow — see
[docs/file-intake.md](docs/file-intake.md) and
[ADR-0004](docs/adr/adr-0004-file-intake-flow.md)), and nine placeholder business
subflows. Flow **definitions** live in the LangFlow Postgres DB (constitution
§7), not on disk — these JSONs are import artifacts, not auto-loaded.
...
[ADR-0004](docs/adr/adr-0004-file-intake-flow.md). Remaining build-phase work
(not done here):
...
5. Wire the nine business subflows in the LangFlow UI and import their real
   ...
   and `ar_file_intake.json` are wired — import the ten subflows first, then
...
shellcheck -x docker/postgres/init/02-ar-agent-db.sh scripts/adapter.selftest.sh scripts/file-intake.selftest.sh
make validate
make test        # includes adapter.selftest.sh (43 checks) + file-intake.selftest.sh (86 checks)
