...
The 16 JSON files here are **LangFlow export skeletons** for the Cosmic AR Agent:

| File | Flow |
|------|------|
| `supervisor.json` | `ar_supervisor` — the **wired** supervisor flow: `SupervisorAgentComponent` (real LangGraph) + fifteen `RunFlow` subflow-as-tool nodes + `ChatInput`/`ChatOutput` (18 nodes / 17 edges). See [docs/supervisor.md](../docs/supervisor.md) and [ADR-0003](../docs/adr/adr-0003-supervisor-runflow-and-adapter.md). |
...
> `ar_invoice_generation.json` are fully wired (their nodes embed the real
> component sources, so they import as working canvases); the nine
> business-subflow skeletons are empty-graph placeholders you import and then
> wire to the bundled components. None are auto-loaded by the `langflow`
> container.
...
`ar_calculation`, `ar_invoice_generation`) with `flow_id_selected=null`, which
LangFlow resolves at runtime **after** the subflow is imported. So:

1. Import the **fifteen subflows first** (the nine placeholders, then wire each
   ...
   `ar_calculation.json`, and `ar_invoice_generation.json` are already wired —
   import them as-is).
