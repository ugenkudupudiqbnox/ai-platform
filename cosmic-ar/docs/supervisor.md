...
`StateGraph[AgentState]` and drives the thirteen reusable AR subflows, which are
...
| Determine execution path | `route` | Maps intent → one of the thirteen subflow tools. On a resume (user text carries an `approval_ref`) records `ar_approval` so the gate's interrupt is reached. |
...
- [ ] Import the thirteen subflow placeholders (incl. the wired `ar_file_intake`,
      `ar_intercompany_sales`, `ar_kitchen_revenue`, and `ar_foodics_processing`),
      then `supervisor.json`; open the supervisor flow so each `RunFlow` node
