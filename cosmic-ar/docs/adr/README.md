...
| [0012](adr-0012-audit-flow.md) | Audit Flow: 16th subflow, validated-JSON `AuditRequest` (the run's execution history/input files/validation reports/calculation results/invoices/approvals/Zoho upload results/execution time/errors/warnings) → validate → collect → synthesize an immutable §13 audit log (append-only AuditRecords, one per artifact + a terminal `audit.summary`) + `ExecutionSummary` → Audit JSON + `WorkflowState`; read-only emission, no §1 gate, no transport; count Fifteen→Sixteen; wired into the supervisor (`RunFlow-ar16`) | Accepted |
| [0013](adr-0013-retire-placeholder-subflows.md) | Retire the seven unimplemented placeholder subflows (`ar_fetch_invoices`/`ar_fetch_receipts`/`ar_match_payments`/`ar_reconcile`/`ar_dunning`/`ar_post_gl`/`ar_reporting`); renumber architecture §4 to the nine implemented subflows (rows 1-9); `FINANCIAL_INTENTS` → `{ar_issue_invoice}`; `supervisor.json` → 12 nodes/11 edges/9 RunFlow; no `AgentState` change; ADRs 0001-0012 left immutable | Accepted |
